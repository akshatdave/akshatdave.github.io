...
def format_authors(authors):
    ...
    for author, link in authors.items():
        if link == "yuzhe":
            author_html.append(f'<span class="yuzhe">{author}</span>')
        else:
...
topics_name_dict = {'Polarization': "Polarization",
               'Time-of-Flight':"Time of Flight",
               'AR-VR': "3D Geometry and Appearance",
               'DeepImagePriors': "Deep Image Priors",
               'NeuralFields': "Neural Fields",
               'Physics-basedML': "Neural Rendering",
               'SeeingAroundCorners': "Seeing Around Corners",
                'SinglePhotonImaging': "Single Photon Imaging",
                'StressAnalysis': "Internal Stress Analysis"}
for topic in topics:
    topic_html_content = []
    ...
    with open(f'pubs_by_topic_{topic}.html', 'w') as f:
...
with open('pubs_selected.html', 'w') as f:
    f.write(sel_html_content)
year_html_content = "\n".join(year_html_content)
with open('pubs_by_date.html', 'w') as f:
